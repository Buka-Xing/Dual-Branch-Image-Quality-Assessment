...
class HighDimProj(nn.Module):
    def __init__(self, in_planes):
        ...
        self.fc1 = nn.Conv2d(in_planes, 1024, 1, bias=False)
        self.relu1 = nn.ReLU()
...
class DBIQA(torch.nn.Module):
    def __init__(self):
        ...
        self.AttenA2 = HighDimProj(in_planes=256)
        self.AttenA3 = HighDimProj(in_planes=512)
        self.AttenA4 = HighDimProj(in_planes=1024)
        self.AttenA5 = HighDimProj(in_planes=2048)

        self.AttenB1 = ChannelAttention(in_planes=64)
        self.AttenB2 = ChannelAttention(in_planes=256)
        self.AttenB3 = ChannelAttention(in_planes=512)
        self.AttenB4 = ChannelAttention(in_planes=1024)
        self.AttenB5 = ChannelAttention(in_planes=2048)
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load(
        './weights/nobias5_subMean_ResNet_PLCC_round3_epoch28.pth',map_location=device))
    model.eval()
