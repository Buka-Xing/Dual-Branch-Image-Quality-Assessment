...
class HighDimProj(nn.Module):
    def __init__(self, in_planes):
        ...
        self.fc1 = nn.Conv2d(in_planes, 340, 1, bias=False)
        self.relu1 = nn.ReLU()
...
class efficientB0(torch.nn.Module):
    def __init__(self, requires_grad=False, resize=False):
        super(efficientB0, self).__init__()
        self.chns = [24, 40, 80, 112, 192, 320]
        self.resize = resize
        self.register_buffer("mean", torch.tensor([0.485, 0.456, 0.406]).view(1, -1, 1, 1))
        self.register_buffer("std", torch.tensor([0.229, 0.224, 0.225]).view(1, -1, 1, 1))

        effb0_pretrained_features = models.efficientnet_b0(pretrained=True).features
        ...
        for x in range(0, 3):
            self.stage1.add_module(str(x), effb0_pretrained_features[x])
        for x in range(3, 4):
            self.stage2.add_module(str(x), effb0_pretrained_features[x])
        for x in range(4, 5):
            self.stage3.add_module(str(x), effb0_pretrained_features[x])
        for x in range(5, 6):
            self.stage4.add_module(str(x), effb0_pretrained_features[x])
        for x in range(6, 7):
            self.stage5.add_module(str(x), effb0_pretrained_features[x])
        for x in range(7, 8):
            self.stage6.add_module(str(x), effb0_pretrained_features[x])
    ...
    def get_features(self, x):
        ...
        h = self.stage6(h)
        h_6 = h
        outs = [x, h_1, h_2, h_3, h_4, h_5, h_6]
...
class DBIQA(torch.nn.Module):
    def __init__(self):
        ...
        self.AttenA2 = HighDimProj(in_planes=40)
        self.AttenA3 = HighDimProj(in_planes=80)
        self.AttenA4 = HighDimProj(in_planes=112)
        self.AttenA5 = HighDimProj(in_planes=192)
        self.AttenA6 = HighDimProj(in_planes=320)

        self.AttenB1 = ChannelAttention(in_planes=24)
        self.AttenB2 = ChannelAttention(in_planes=40)
        self.AttenB3 = ChannelAttention(in_planes=80)
        self.AttenB4 = ChannelAttention(in_planes=112)
        self.AttenB5 = ChannelAttention(in_planes=192)
        self.AttenB6 = ChannelAttention(in_planes=320)
    ...
    def forward_A(self, x):
        ...
        h_relu6_4 = self.AttenA6(x[6].clone())
        return [x[0], h_relu1_2, h_relu2_2, h_relu3_3, h_relu4_3, h_relu5_4, h_relu6_4]
    ...
    def forward_B(self, x):
        ...
        h_relu6_4 = self.AttenB6(x[6].clone())
        return [x[0], h_relu1_2, h_relu2_2, h_relu3_3, h_relu4_3, h_relu5_4, h_relu6_4]
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load(
        './weights/nobias5_subMean_efficientB0_PLCC_round2_epoch3.pth',map_location=device))
    model.eval()

    net   = efficientB0().to(device)
    net.eval()
